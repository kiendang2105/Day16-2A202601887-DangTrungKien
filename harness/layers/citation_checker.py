...
class CitationChecker(Middleware):
    ...
    def after_agent(self, ctx, report):
        ...
        if not isinstance(claims, list) or not claims or not ctx.corpus:
            return report
        ...
        for claim in claims:
            ...
            if current is not None and self._matches_line(text, current):
                continue
            for doc in ctx.corpus.docs:
                if doc.body in ctx.observed_text and self._matches_line(text, doc):
    ...
    @staticmethod
    def _matches_line(text, doc):
        return isinstance(text, str) and any(text in line for line in doc.body.splitlines())
